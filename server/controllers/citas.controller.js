...
export const createCita = async (req, res, next) => {
  try {
    ...
    const { idpsicologo, idhorario, online, motivo } = req.body;
    ...
    if (citaPendiente) {
      return res
        .status(400)
        .json({ message: "Ya tienes una cita pendiente con este psicólogo" });
    }

    console.log(
      `[createCita] Verificando horario. Recibido idhorario: ${idhorario}, Tipo: ${typeof idhorario}`
    );
    ...
    if (isNaN(idHorarioNumerico)) {
      return res
        .status(400)
        .json({ message: "ID de horario inválido (no es número)." });
    }
    ...
    if (!horario) {
      console.error(
        `[createCita] Horario no encontrado en DB para idhorario (numérico): ${idHorarioNumerico}`
      );
    ...
    const fecha = moment().day(diaSemana).isBefore(moment())
      ? moment().day(diaSemana + 7)
      : moment().day(diaSemana);
    ...
    if (citaConflicto) {
      return res.status(400).json({
        message:
          "El psicólogo ya tiene este horario ocupado con otro paciente.",
      });
    }
    ...
    if (idusuario_psicologo) {
      console.log(
        `[createCita] Se encontró idusuario ${idusuario_psicologo} para idpsicologo ${idpsicologo}.`
      );
      ...
      try {
        ...
        console.log(
          `[createCita] Notificación en DB creada para psicólogo (usuario ID ${idusuario_psicologo})`
        );
      } catch (dbNotifyError) {
        console.error(
          "[createCita] Error al guardar notificación en DB:",
          dbNotifyError
        );
      }
      ...
      console.warn(
        `[createCita] No se pudo encontrar el idusuario para el psicólogo con idpsicologo ${idpsicologo}. No se envió notificación WS.`
      );
    }
...
export const getCitasPsicologo = async (req, res, next) => {
  try {
    ...
    if (!psicologo) {
      return res
        .status(404)
        .json({ message: "Psicólogo no encontrado para este usuario." });
    }
...
export const updateCita = async (req, res, next) => {
  try {
    ...
    const { iddiagnostico, comentario, estado } = req.body;
    ...
    if (!psicologo) {
      return res
        .status(403)
        .json({ message: "Acción no permitida para este usuario." });
    }
    ...
    if (cita.idpsicologo !== idpsicologo_verificado) {
      return res.status(403).json({
        message: "Este psicólogo no está autorizado para modificar esta cita.",
      });
    }
    ...
    const diagnosticoNombre = diagnostico
      ? diagnostico.nombre
      : "Sin diagnóstico asignado";

    const mensaje =
      estado === "Realizado"
        ? `Tu cita del ${moment(result.fecha).format(
            "DD/MM/YYYY"
          )} ha sido marcada como realizada. Diagnóstico: ${diagnosticoNombre}. Comentario: ${
            comentario || "Ninguno"
          }.`
        : estado === "Ausente"
        ? `Tu cita del ${moment(result.fecha).format(
            "DD/MM/YYYY"
          )} ha sido marcada como ausente.`
        : `El estado de tu cita del ${moment(result.fecha).format(
            "DD/MM/YYYY"
    ...
    try {
      ...
      console.log(
        `[updateCita] Notificación DB creada para paciente ${idusuario_paciente}`
      );
    } catch (dbNotifyError) {
      console.error(
        "[updateCita] Error al guardar notificación para paciente en DB:",
        dbNotifyError
      );
    }
