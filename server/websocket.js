  ...
  wss.on("connection", (ws, req) => {
    ...
    try {
      ...
      if (userRole === "Psicologo") {
        ...
        ws.send(
          JSON.stringify({
            type: "auth_success",
            message: "Conectado y autenticado como Psicólogo.",
          })
        );
      } else {
        console.log(
          `[WebSocket] Usuario ${userId} (${userRole}) conectado, pero no rastreado para notificaciones de cita.`
        );
        // Opcional: Enviar mensaje informativo
        ws.send(
          JSON.stringify({
            type: "connection_ok",
            message: "Conectado, pero no recibirá notificaciones de citas.",
          })
        );
      }
    } catch (err) {
      ...
      console.error(
        `[WebSocket] Autenticación fallida (${reason}). Cerrando conexión. Error: ${err.message}`
      );
    ...
    ws.on("close", (code, reason) => {
      ...
      if (
        userRole === "Psicologo" &&
        userId &&
        connectedPsychologists.has(userId)
      ) {
        console.log(
          `[WebSocket] Psicólogo ${userId} desconectado. Código: ${code}, Razón: ${reasonString}`
        );
        ...
        if (connections.size === 0) {
          ...
          console.log(
            `[WebSocket] Última conexión cerrada para Psicólogo ${userId}.`
          );
        }
      } else if (userId) {
        console.log(
          `[WebSocket] Usuario ${userId} (${userRole}) desconectado. Código: ${code}, Razón: ${reasonString}`
        );
      } else {
        console.log(
          `[WebSocket] Cliente no autenticado desconectado. Código: ${code}, Razón: ${reasonString}`
        );
      }
    });
    ...
    ws.on("error", (error) => {
      console.error(
        `[WebSocket] Error en conexión (Usuario ${userId || "desconocido"}):`,
        error
      );
      ...
      if (
        ...
        if (connections.has(ws)) {
          ...
          console.log(
            `[WebSocket] Conexión con error eliminada para Psicólogo ${userId}.`
          );
        }
      }
    ...
    ws.on("message", (message) => {
      ...
      console.log(
        `[WebSocket] Mensaje de ${userId} (${userRole}): ${messageString}`
      );
      try {
        ...
        console.log(
          "[WebSocket] Mensaje recibido no es JSON o tipo desconocido."
        );
      }
    });
  });
...
export async function notifyPsychologist(
  ...
  if (!psychologistUserId) {
    console.warn(
      "[WebSocket Notify] Se intentó notificar sin psychologistUserId."
    );
    return;
  }
  ...
  if (connectedPsychologists.has(psychologistUserId)) {
    ...
    if (connections.size > 0) {
      console.log(
        `[WebSocket Notify] Enviando notificación a ${connections.size} conexiones del psicólogo ${psychologistUserId}`
      );
      ...
      connectionsToSend.forEach((client) => {
        if (client.readyState === 1) {
          ...
          client.send(message, (err) => {
            if (err) {
              console.error(
                `[WebSocket Notify] Error enviando a una conexión del psicólogo ${psychologistUserId}:`,
                err
              );
          ...
          console.warn(
            `[WebSocket Notify] Eliminando conexión no abierta para psicólogo ${psychologistUserId}`
          );
          connections.delete(client);
          if (connections.size === 0) {
            connectedPsychologists.delete(psychologistUserId);
      ...
      if (connections.size === 0) {
        console.log(
          `[WebSocket Notify] No quedaron conexiones activas para ${psychologistUserId} después del envío.`
        );
        connectedPsychologists.delete(psychologistUserId);
      }
    } else {
      ...
      console.log(
        `[WebSocket Notify] No hay conexiones activas para psicólogo ${psychologistUserId}, aunque la entrada existía. Limpiando.`
      );
      connectedPsychologists.delete(psychologistUserId);
    }
  } else {
    console.log(
      `[WebSocket Notify] Psicólogo ${psychologistUserId} no tiene conexiones WebSocket activas.`
    );
  }
}
...
export async function getUserIdForPsychologist(idpsicologo) {
  ...
  try {
    ...
    console.error(
      `[DB Error] Buscando idusuario para idpsicologo ${idpsicologo}:`,
      error
    );
    return null;
  }
